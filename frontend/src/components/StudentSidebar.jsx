    ...
    <>
      ...
      {/* Sidebar */}
        <div className={`fixed inset-y-0 left-0 z-40 w-64 bg-cyan-700/85 text-white transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:inset-0`}>
          ...
          <Link
            to={`profile?studentId=${studentId}`}
          ...
          <Link
            to={`details?studentId=${studentId}`}
            className={`block px-4 py-2 rounded ${
              location.pathname === '/student/dashboard/details'
            ? 'bg-cyan-900'
            : 'hover:bg-cyan-700'
            }`}
            onClick={() => setIsOpen(false)}
          ...
          <Link
            ...
            className={`block px-4 py-2 rounded ${
              location.pathname === '/student/dashboard/jobs'
            ? 'bg-cyan-900'
            : 'hover:bg-cyan-700'
            }`}
            onClick={() => setIsOpen(false)}
